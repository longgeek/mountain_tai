...
def get_images(host_ip, host_port, user_id=None):
    ...
    if host_object[0]:
        ...
        for image in images:
            ...
            # 以 MB 为单位
            size = image['Size'] / 1024000.0
            virtual_size = image['VirtualSize'] / 1024000.0
...
def delete_images(host_ip, host_port, image, user_id=None):
    ...
    host_object = connect_host(host_ip, host_port)
    try:
        host_object[2].remove_image(image)
        return (1, '', '%s has been deleted!')
